...
def get_challenge():
    ...
    try:
        ...
        return generate_response("success", "Challenge generated successfully", challenge=challenge)
    ...
    try:
        ...
        if stored_result:
            ...
            if client_response == expected_response:
                ...
                return generate_response("success", "Authentication successful", session_id=session_uuid)
            else:
...
@app.route(f"{BASE_URL_PREFIX}/check-session", methods=['POST'])
def check_session():
    """
    ...
    try:
        ...
        if session_id:
            ...
            return generate_response("success", "Session is active")
        else:
...
def get_node_provision():
    ...
    # Retrieve provisioning information from the MongoDB database (assuming `provisioning` collection)
    ...
    # Retrieve camera information from the MongoDB database (assuming `cameras` collection)
    camera_info = db.cameras.find_one({"camera_id": camera_id})
    ...
    return generate_response("success", "Node provisioned successfully", 
                            ...
                            camera_password=camera_password), 200
